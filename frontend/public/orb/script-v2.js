...
window.addEventListener("message", (event) => {
  ...
  if (data.type === "anta-audio") {
    material.uniforms.u_audioLow.value = Number(data.bass) || 0;
    material.uniforms.u_audioMid.value = Number(data.mid) || 0;
...
setupTextTexture();
animate();
const initialPreset = { color1: [1.0, 0.70, 0.16], color2: [1.0, 0.46, 0.18], color3: [0.55, 0.16, 0.48], background: [0.0, 0.0, 0.0] };
...
material.uniforms.u_showText.value = false;
material.uniforms.u_isMonochrome.value = false;
setTimeout(() => {
  addRipple(getW() / 2, getH() / 2, 1.5);
}, 500);