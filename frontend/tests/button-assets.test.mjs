...
try {
  ...
  await page.waitForSelector('.dock button', { timeout: 30000 });

  const data = await page.evaluate(() => [...document.querySelectorAll('.dock button')].map((button) => {
  ...
  console.log('✓ Button assets match: angled blue HUD chips with glowing icons');
} finally {
  await browser.close();
}
